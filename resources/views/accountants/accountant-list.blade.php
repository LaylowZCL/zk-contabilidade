...
@section('body')
    ...
    @section('content')
        @component('components.breadcrumb')
            ...
            @endslot
            @slot('pagetitle')
                {{ __('translation.Accountant List') }}
            @endslot
        @endcomponent
        ...
        <div class="row pb-4 gy-3">
            <div class="col-sm-4">
                <button class="btn btn-primary addPayment-modal" data-bs-toggle="modal"
                    data-bs-target="#addAccountantModal"><i class="las la-plus me-1"></i>
                    {{ __('translation.Add Accountant') }}</button>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive table-card">
                            ...
                            @foreach ($accountants as $key => $accountant)
                                <input type="hidden" id="first_name_{{ $accountant->id }}"
                                    value="{{ @$accountant->first_name }}">
                                <input type="hidden" id="last_name_{{ $accountant->id }}"
                                    value="{{ @$accountant->last_name }}">
                                <input type="hidden" id="email_{{ $accountant->id }}" value="{{ @$accountant->email }}">
                                <input type="hidden" id="username_{{ $accountant->id }}"
                                    value="{{ @$accountant->username }}">
                                <input type="hidden" id="country_{{ $accountant->id }}"
                                    value="{{ @$accountant->country }}">
                                <input type="hidden" id="mobile_number_{{ $accountant->id }}"
                                    value="{{ @$accountant->mobile_number }}">
                                <input type="hidden" id="created_by_{{ $accountant->id }}"
                                    value="{{ @$accountant->created_by }}">
                                <input type="hidden" id="profile_image_{{ $accountant->id }}"
                                    value="{{ URL::asset('build/images/users/' . $accountant->profile_image) }}">
                            @endforeach
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content border-0">
                    ...
                    <div class="modal-body p-4">
                        <form method="POST" action="{{ url('add-accountant') }}" autocomplete="off" id="clientform"
                                ...
                                <div class="col-lg-12">
                                        ...
                                        <div class="position-relative d-inline-block">
                                            <div class="position-absolute bottom-0 end-0">
                                                <label for="member-image-input" class="mb-0" data-bs-toggle="tooltip"
                                                    data-bs-placement="right" title="Select Accountant Image">
                                                    <div class="avatar-xs">
                                                        <div
                                    ...
                                    <div class="hstack gap-2 justify-content-end">
                                        ...
                                        <button type="submit" class="btn btn-success"
                                            id="addNewClient">{{ __('translation.Add Accountant') }}</button>
                                    </div>
                                </div>
                            </div>
                        </form>
        ...
        <div class="modal fade" id="editAccountantModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content border-0">
                    ...
                    <div class="modal-body p-4">
                        <form method="POST" action="{{ url('update-accountant') }}" autocomplete="off"
                            id="accountantform" class="needs-validation" enctype="multipart/form-data" novalidate>
                            @csrf
                            <div class="row">
                                <div class="col-lg-12">
                                    <input type="hidden" id="memberid-input" class="form-control" value="">
                                    ...
                                    <div class="text-center mb-2">
                                        <div class="position-relative d-inline-block">
                                            <div class="position-absolute bottom-0 end-0">
                                                <label for="editAccountantProfile" class="mb-0"
                                                    data-bs-toggle="tooltip" data-bs-placement="right"
                                                    title="Select Accountant Image">
                                                    <div class="avatar-xs">
                                                        <div
                                                            class="avatar-title bg-light border rounded-circle text-muted cursor-pointer">
                                                            <i class="ri-image-fill"></i>
                                                ...
                                                <input
                                                    class="form-control d-none @error('profile_image') is-invalid @enderror"
                                                    value="" id="editAccountantProfile" type="file"
                                                    name="profile_image" accept="image/png, image/gif, image/jpeg"
                                    ...
                                    <div class="hstack gap-2 justify-content-end">
                                        <button type="button" class="btn btn-light"
                                            data-bs-dismiss="modal">{{ __('translation.Close') }}</button>
                                        <button type="submit" class="btn btn-success"
                                            id="addNewClient">{{ __('translation.Update Accountant') }}</button>
                                    </div>
                                </div>
                            </div>
                        </form>
                                ...
                                <div class="col-lg-12">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-6 mb-2 mt-2">
                                            <label for="country" class="form-label">{{ __('translation.Country') }}<span
                                                    class="text-danger">*</span></label>
                                            <input type="text" class="form-control" id="viewCountry" name="country"
                                                readonly>
                                        </div>
                                    </div>
    ...
    @section('scripts')
        ...
        <script>
            ...
            function viewAccountantData(accountantId) {
                if (accountantId) {
                    document.getElementById('viewFirstName').value = document.getElementById('first_name_' +
                        accountantId).value;
                    document.getElementById('viewLastName').value = document.getElementById('last_name_' + accountantId)
                        .value;
                    document.getElementById('viewEmail').value = document.getElementById('email_' + accountantId).value;
                    document.getElementById('viewCountry').value = document.getElementById('country_' + accountantId)
                        .value;
                    document.getElementById('viewUsername').value = document.getElementById('username_' + accountantId)
                        .value;
                    document.getElementById('viewMobileNumber').value = document.getElementById('mobile_number_' +
                        accountantId).value;
                    document.getElementById("viewAccountantImage").src = document.getElementById('profile_image_' +
                        accountantId).value;
                }
            }
            ...
            function editAccountantData(accountantId) {
                if (accountantId) {
                    document.getElementById('editFirstName').value = document.getElementById('first_name_' +
                        accountantId).value;
                    document.getElementById('editLastName').value = document.getElementById('last_name_' + accountantId)
                        .value;
                    document.getElementById('editEmail').value = document.getElementById('email_' + accountantId).value;
                    document.getElementById('editCountry').value = document.getElementById('country_' + accountantId)
                        .value;
                    document.getElementById('editUsername').value = document.getElementById('username_' + accountantId)
                        .value;
                    document.getElementById('editMobileNumber').value = document.getElementById('mobile_number_' +
                        accountantId).value;
                    document.getElementById("editAccountantImage").src = document.getElementById('profile_image_' +
                        accountantId).value;
