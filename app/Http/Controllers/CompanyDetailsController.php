            ...
            if ($request->ajax()) {
                return Datatables::of($companies)
                    ...
                    ->addColumn('company_name', function($row){
                        $url=asset("build/images/logo/$row->logo");
                        $company_name = '
                            <div class="d-flex align-items-center">
                                <div class="flex-shrink-0 me-3 avatar-sm">
                                    <div class="avatar-title bg-light rounded"> <img src="'.$url.'" alt="'.$row->company_name.'" class="avatar-xs"> </div>
                                </div>
                                <div class="flex-grow-1">
                                    <h6 class="fs-16 mb-1">'.$row->company_name.'</h6>
                                </div>
                            </div>
                            ';
                        return $company_name;
        ...
        if ($authUser->hasAccess('company.add')) {
            ...
            try {
                ...
                if ($request->logo != null) {
                    ...
                    $file->move(public_path('build/images/logo/'), $imageName);
                }
                // company details add
    ...
    public function updateCompany(Request $request) {
        ...
        if ($user->hasAccess('company.edit')) {
            ...
            try {
                ...
                if ($request->logo != null) {
                    $isAvailable = 'build/images/logo/.' . $company->logo;
