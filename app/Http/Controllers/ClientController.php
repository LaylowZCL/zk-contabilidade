            ...
            if ($request->ajax()) {
                return Datatables::of($clients)
                    ...
                    ->addColumn('client_name', function($row){
                        $url=asset("build/images/users/".$row['profile_image']);
                        $client_name = '
                    ...
                    ->addColumn('action', function($row){
                        $action = '
                            <ul class="list-inline hstack gap-1 mb-0">
                                ...
                                <button class="btn btn-soft-info btn-sm d-inline-block edit-button" data-bs-toggle="modal" onclick="editClientData('. $row['id'] .')" data-bs-target="#editClientModal" data-edit-id="'.$row['id'].'"><i class="las la-pen fs-17 align-middle"></i></button>
                                </li>
        ...
        if ($authUser->hasAccess('client.add')) {
            ...
            try {
                if ($request->profile_image != null) {
                    ...
                    $file->move(public_path('build/images/users/'), $imageName);
                }
                ...
                if($user) {
                    ...
                    if($companyDetails->save()) {
                        ...
                        $mailArray = [$request->email];
                        Mail::send('emails.verify-account', ['email' => $request->email, 'password' => $request->password, 'token' => $token, 'app_url' => $app_url, 'role' => $role->slug], function ($message) use ($mailArray,$app_name) {
    ...
    public function updateClient(Request $request) {
        ...
        if ($user->hasAccess('client.edit')) {
            ...
            try {
                ...
                if ($request->profile_image != null) {
                    $isAvailable = 'build/images/users/.' . $client->profile_image;
