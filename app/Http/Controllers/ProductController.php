            ...
            if ($request->ajax()) {
                return Datatables::of($products)
                    ...
                    ->addColumn('product_name', function ($row) {
                        $url = asset("build/images/products/" . $row->product_image);
                        $product_name = '
                        ...
                        $action = '
                            <ul class="list-inline hstack gap-1 mb-0">
                                ...
                                <li class="list-inline-item edit" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Edit">
                                    <a href="'.url('edit-product').'/'.$row->id.'" class="btn btn-soft-info btn-sm d-inline-block edit-button"  data-edit-id="' . $row->id . '"><i class="las la-pen fs-17 align-middle"></i></a>
                                </li>
                                <li class="list-inline-item" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Change Status">
                                <a href="'.url('delete-product').'/'.$row->id.'" onclick="return confirm(`Are you sure to change product status?`)" type="button" class="btn btn-soft-danger btn-sm d-inline-block remove-btn"  data-remove-id="' . $row->id . '"><i class="las la-sync fs-17 align-middle"></i></a>
                                </li>
                            </ul>
                        ';
                        return $action;
        ...
        if ($authUser->hasAccess('product.add')) {
            ...
            try {
                if ($request->product_image != null) {
                    ...
                    $file->move(public_path('build/images/products/'), $imageName);
                }
        ...
        if ($authUser->hasAccess('product.edit')) {
            ...
            try {
                ...
                if ($request->product_image != null) {
                    ...
                    $old_thumb_destination = 'build/images/products/' . $product->product_image;
    ...
    public function deleteSubImage(Request $request)
    {
        ...
        if ($user->hasAccess('product.delete')) {
            try {
                ...
                $old_image_destination = 'build/images/products/' . $productImage->image;
